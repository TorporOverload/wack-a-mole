...
$(window).on("mousemove", function (e) {
  ...
  const hammer = $(".hammer");
  hammer.css("left", e.pageX);
  hammer.css("top", e.pageY);
});
...
$(window).on("click", function (e) {
  ...
  const hammer = $(".hammer");
  hammer.css("transform", "translate(-20px, -30px) rotate(-45deg)");
  setTimeout(() => {
    hammer.css("transform", "translate(-20px, -30px) rotate(0deg)");
  }, 100);
});
